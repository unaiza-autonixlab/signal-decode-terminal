      ...
      <section className="max-w-5xl mx-auto py-14 px-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { value: "$2,700", suffix: "", subtext: "", label: "Monthly savings vs previous content ops spend ($4,200 → $1,500)", metric: "METRIC_04" },
          ].map((stat) => (
            <div key={stat.metric} className="stat-card">
              <div className="text-[10px] text-terminal-green mb-1">{stat.metric}</div>
              {'value' in stat ? (
                <div className="text-4xl font-bold text-primary mb-1 tabular-nums leading-tight">{stat.value}<span className="text-2xl block">{stat.subtext || stat.suffix}</span></div>
              ) : (
      ...
      <section className="max-w-5xl mx-auto py-14 px-6 border-t border-border">
        <div className="terminal-window p-8 md:p-10">
          <div className="border-l-2 border-primary pl-6 py-4">
            <blockquote className="text-xl md:text-2xl italic leading-relaxed text-center md:text-left">
